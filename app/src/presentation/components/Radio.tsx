...
export default (props: RadioProps): JSX.Element => {
  const id = `checkbox-${Math.random().toString(36).slice(2)}`

  return (
    <label
      htmlFor={id}
      className='flex items-center gap-2 cursor-pointer text-[#757a7e] has-[:checked]:text-[#24282b]'
    >
      <input
        type='radio'
        id={id}
        ...
        className='appearance-none w-5 h-5 border-2 border-[#eaebed] rounded-full cursor-pointer relative 
        checked:bg-[#4b4bce] 
        checked:border-[#4b4bce] 
        checked:w-3
        checked:h-3
        checked:mx-1
        transition-all 
        duration-200 
        [box-shadow:_0_0_0_3px_#ffffff] checked:[box-shadow:_0_0_0_3px_#ffffff,_0_0_0_5px_#4b4bce]
'
      />
      {props.label && (
        <span className='text-[13px] font-semibold flex items-center gap-2'>
          <svg viewBox='0 0 24 24' fill='none' className='w-[18px]'>
            <path
